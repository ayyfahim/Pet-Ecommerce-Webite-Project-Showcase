<div class="tab-pane fade" id="ingredients" role="tabpanel" aria-labelledby="nutrition-tab">
    <div class="row">
        <div class="col-md-12">
            <div class="form-group">
                <label>Ingredients Description</label>
                <textarea class="form-control tinymce-text" style="height: 300px"
                    name="ingredients_description">{{isset($product)?$product->info->ingredients_description:''}}</textarea>
    ...
    <div class="nutrition_fact_serving-wrapper">
        ...
        @foreach(array_values($product->nutrition_facts_serving) as
        $nutrition_factServingKey=>$nutrition_factServingItem)
    ...
    <div class="text-right">
        <button type="button" class="btn btn-sm btn-light add-nutrition_facts_serving-button ajax_without_form"
            ...
            data-method="POST" data-append="1" data-append-view-wrapper=".nutrition_fact_serving-wrapper">Add Serving
        </button>
    </div>
    <hr class="my-2">
    ...
    <div class="text-right">
        <button type="button" class="btn btn-sm btn-light add-nutrition_facts_serving-button ajax_without_form"
            ...
            data-key="{{(isset($product) && $product->nutrition_facts_serving)?sizeof($product->nutrition_facts_serving)-1:0}}"
            data-method="POST" data-append="1" data-append-view-wrapper=".nutrition_fact_weight-wrapper">Add Weight
        </button>
    </div>
</div>