...
const ProductList = ({ slug, frontendData, catData }) => {
  let category_id = catData?.id;
  let category = catData;
  ...
  const filterBrandIds =
    (filterBrands && filterBrands.length && filterBrands?.filter((data) => data?.selected == true)?.map(({ id }) => id))
      ?.length > 0
  ...
  return (
    <div className={style.wrapper}>
      <Banner
        title={catData?.banner_title || 'Category'}
        description={catData?.banner_description || null}
...
export const getServerSideProps = createGetServerSidePropsFn(ProductList, async ({ params }) => {
  ...
  const response2 = await getRequest(`/categories/single/${params?.slug}`);

  return {
    props: {
      slug: params?.slug || '',
      frontendData: response?.page ?? [],
      catData: response2?.category ?? [],
    },
  };
});
